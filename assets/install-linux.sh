#!/usr/bin/env bash
#
# Install probe-shell's icon + desktop entry on Linux so the GNOME/Ubuntu dock and
...
# running window to its .desktop file via the window's app_id — probe-shell sets
# that to "probe-shell" (slint::set_xdg_app_id), and this script's StartupWMClass
# matches it.
#
# Usage:
#   ./install-linux.sh [/path/to/probe-shell-binary]
# You normally don't need an argument: when run from inside a release package
# (the `probe-shell` binary sits next to this script) it is picked up automatically.
# In the source tree it falls back to ./target/release/probe-shell.
...
if [ -n "${1:-}" ]; then
    BIN="$1"
elif [ -x "$SCRIPT_DIR/probe-shell" ]; then
    BIN="$SCRIPT_DIR/probe-shell"
else
    BIN="$SCRIPT_DIR/../target/release/probe-shell"
fi
...
if [ ! -x "$BIN" ]; then
    echo "error: probe-shell binary not found: $BIN" >&2
    echo "Run this script from the extracted release folder (it sits next to the" >&2
    echo "'probe-shell' binary), or pass the binary path as an argument." >&2
    exit 1
fi
...
if [ -f "$ICON_SRC" ]; then
    install -m644 "$ICON_SRC" "$ICON_DIR/probe-shell.png"
else
    echo "warning: icon not found ($ICON_SRC); the desktop entry will use a generic icon" >&2
fi

cat > "$APP_DIR/probe-shell.desktop" <<EOF
[Desktop Entry]
Type=Application
Name=Probe Shell
...
StartupNotify=true
StartupWMClass=probe-shell
EOF
chmod 644 "$APP_DIR/probe-shell.desktop"
...
echo "Installed:"
echo "  icon    -> $ICON_DIR/probe-shell.png"
echo "  desktop -> $APP_DIR/probe-shell.desktop"
echo "  exec    -> $BIN"
echo
