...
- 输入框统一支持右键剪切、复制、粘贴和全选；终端隐藏输入框保持原有输入与 IME 逻辑。
- 密码和私钥口令输入框新增显示/隐藏按钮；失焦自动隐藏，显示状态下仍禁止复制和剪切。
- 修复空白主机、非法端口和切换认证方式时旧凭据残留等会话保存问题。
- 断线或连接中不再把本机数据显示为服务器资源，并补齐中英文界面翻译。
- 明确 Windows portable ZIP 与 MSI 的区别，并增强 MSI 交互安装向导校验。

## v0.7.7

- 标签构建失败，未生成正式发布物；实际修复内容随 v0.7.8 发布。
...
- 快捷访问 `+` 只保留“快速添加勾选目录”机制；未勾选时提示去齿轮设置里手动添加。
- 齿轮改为快捷目录管理弹窗：可手动填写目录地址与备注，也可在弹窗里删除默认/自定义快捷入口。
- 上一级目录 `↑` 放到快捷访问栏齿轮右侧，减少跨栏点击。
- 设备类型判断升级为多信号画像：名称、备注、品牌关键词、IP 网关规律、协议共同评分；低可信时不乱猜，仍显示 SSH/Telnet/Serial。
- 新增锐捷/小米/TP-Link/华为/华硕/光猫/NAS/OpenWrt/Docker 等识别标签；路由/OpenWrt 画像默认偏向 SSH 文件浏览，NAS/Linux 默认偏向 SFTP。

## v0.6.5-probe4

- 快捷访问栏进一步收敛：`根` 改为 `根目录`，HOME/ETC/VAR/TMP/WWW/OVER 字号与按钮宽度缩小，减少遮挡。
- 快捷访问栏新增上一级 `↑` 入口，常用目录旁边可直接返回父目录。
- `+` 快捷目录入口恢复可用：有勾选目录时加入勾选搜索范围；无勾选时弹出输入框，可填写目录地址和备注。
- 设置齿轮进入快捷目录管理模式，快捷目录右上角显示 `×`，点击即可隐藏该快捷入口。
- 修复根目录或父级选中后子目录“勾选关不掉”的问题：点击继承勾选的子目录会清空当前搜索范围。
- 目录树展开箭头更保守：未知/无下级目录不再默认显示下拉箭头，避免单文件/空目录看起来可以展开。
- 右键刷新会在底部状态栏显示“正在刷新”，避免用户误以为没反应。
- 搜索状态刷新节流缩短，搜索中状态反馈更快。

## v0.6.5-probe1

## v0.6.5-probe3

- 修复空搜索范围被解析为根目录导致目录树默认全勾选的问题。
- 合并 SFTP 搜索/停止按钮：搜索中按钮自动变为“停止”。
- 增加更明显的工具栏悬浮说明气泡，鼠标停在日志/搜索/复制/上传等图标时能看到用途。
- 缩小目录树勾选框，并保持点击勾选框可独立勾选/取消。
- 右侧文件列表新增“权限”列，和名称、大小、修改时间一起显示。
- 快捷访问栏字体和按钮尺寸收敛，新增“+”与管理入口占位。
- 修复目录树名称区域右键无菜单的问题。
...
- 勾选文件后显示轻量批量操作条：复制路径、下载、清除选择；删除仍保留确认流程，避免误操作系统目录。
- 复制选中路径会一次性复制多行远程路径，方便发日志或排查。

## v0.6.4-probe4

- SFTP 工具栏小图标增加悬浮说明：鼠标悬浮时底部状态栏显示“搜索 / 停止 / 复制路径 / 粘贴跳转 / 上传 / 刷新 / 折叠 / 操作日志”等用途。
- 新增本地操作日志 `log/operations.log`：记录上传、下载、搜索、停止搜索、新建、删除、重命名、权限修改、查看、编辑、保存等文件操作的时间、目标路径和状态。
- SFTP 工具栏新增“操作日志”按钮，可直接打开本机 `operations.log` 查看历史行为。
- 操作日志只保存操作行为与路径，不保存文件内容、密码或私钥。

## v0.6.4-probe3

- 重新理清 SFTP 目录树点击区域：勾选框只负责递归选择/搜索范围，展开箭头只负责展开/折叠，文件夹名称单击只选中、双击才打开目录。
- 目录树勾选改为递归范围：勾选 `/etc` 后，已加载的下级目录和当前文件列表会自动显示为勾选；后续展开到该范围内的目录也会继承勾选状态。
- 地址栏只表示“当前打开目录”：右键、勾选、单击选中不会再偷偷改变地址栏；手动输入路径并按 Enter 才会跳转。
- 底部状态栏同时显示当前 SFTP 状态、已选中目录、搜索范围，减少“当前到底操作哪个目录”的混乱。
- 树节点展开不再触发右侧文件列表刷新，避免展开结构时误跳目录。

## v0.6.4-probe2

- 左侧 SSH/Telnet 会话列表改为安全的单击/双击分离：单击只切换到已有标签或打开一次，双击强制新开一个 SSH/Telnet 标签。
- 双击不会再同时触发“切换已有标签”和“新建连接”两条路径，避免失败会话/连接中会话出现竞态闪退。
- 新开的重复会话标签自动追加 `#2/#3`，便于区分同一主机的多个页面。
- Rust 侧只对“真实新建标签”做防抖，正常单击切换已有页面不被 900ms 防抖误挡。
- 保留 v0.6.4-probe1 的异步搜索与停止搜索能力，搜索任务不应阻塞目录展开、刷新和页面切换。

## v0.6.4-probe1

- SFTP 搜索改为后台任务，避免递归搜索时卡住目录展开、刷新和页面切换。
- 新增“停止”搜索按钮，可取消正在运行的递归文件搜索。
- 搜索状态栏显示当前正在扫描的目录和已扫描目录数量，明确“搜的是哪里”。
- SFTP 搜索任务会自动取消上一次未完成搜索，避免多次搜索互相抢 UI 状态。
- Bug 日志核查：本次上传日志只记录 SSH 正常关闭，没有新的 panic。
...
- **Project rename to Probe Shell / 项目重命名为 Probe Shell.** Cargo package, app id, desktop file, macOS bundle, Windows resources, release workflow, README links and UI copy now use `probe-shell` / `Probe Shell`.
- **Win12-style glass theme / Win12 风格玻璃主题.** Updated shared design tokens: larger rounded corners, blue-violet accent, softer borders, darker terminal-safe base, and a sci-fi `builtin:tech` wallpaper as the default.
- **AccessKit focus crash fix / 修复 AccessKit 焦点崩溃.** `TerminalView` no longer calls `ime-input.focus()` directly from `init` / `changed visible`; focus is deferred with a one-shot timer to avoid `RefCell already borrowed` during accessibility-tree rebuilds.
- **First app.rs split / 第一步拆分 app.rs.** Pure shared state types were moved to `src/app_state.rs`. The remaining `app.rs` is still the Slint callback hub and can be split further by callback domain later.
...
- **终端内容随窗口缩放重排 (reflow) (#169)。** 此前拖动窗口宽度时,已打印的内容会被截断(变窄丢右半、
  ...
  **Terminal content reflows on window resize (#169).** Dragging the width used to clip already-printed
  lines. probe-shell now retains a capped (2 MiB) copy of the byte stream and replays it at the new width,
...
- **记录 RUSTSEC-2026-0154 不可达,暂不升级 russh (#151)。** 该 DoS 在 russh 的 ssh-agent 帧解析,
  probe-shell 完全不用 ssh-agent,漏洞代码路径在本二进制里不可达、不可利用;而唯一修复版
  ...
  **Documented RUSTSEC-2026-0154 as unreachable; holding russh at 0.49 (#151).** The DoS is in russh's
  ssh-agent frame parsing, which probe-shell never uses — the path is dead code here. The only patched
...
- **修正 macOS 安装说明(#135)。** README 写的是 `tar -xzf …macos-*.tar.gz` + 裸 `probe-shell`
  二进制,但实际发布产物是 `.zip` + `probe-shell.app` 应用包,三条命令全对不上。已改为:解压
  ...
  **Fixed the macOS install instructions (#135).** The README said to `tar -xzf …macos-*.tar.gz`
  and run a bare `probe-shell` binary, but the release artifact is a `.zip` containing
  `probe-shell.app`. Updated to: unzip → optionally move to `/Applications` → clear
...
- **macOS 26 blank text — switch the default CJK UI font to one femtovg can render
  ...
  A `PROBE_SHELL_UI_FONT="<family>"` env var can force any family without a rebuild.
  **修复 macOS 26 文字全白——默认中文界面字体改用 femtovg 能渲染的字体 (#129, #108)。**
  ...
  `PROBE_SHELL_UI_FONT="<字体名>"` 免重编强制指定任意字体。
...
- **Hide the shell-integration command from the terminal.** probe-shell injects a
  ...
  **隐藏 shell 集成注入命令。** probe-shell 连接时会注入一行 `PROMPT_COMMAND`
...
- `install-linux.sh` now auto-detects the `probe-shell` binary sitting next to it
  ...
  `install-linux.sh` 现在自动识别发布包里同目录的 `probe-shell`,无需传参即可使用
...
- **Linux desktop integration** — `assets/probe-shell.desktop` + `install-linux.sh`
  and an `xdg_app_id` so the GNOME/Ubuntu dock shows the app icon on Wayland.
  **Linux 桌面集成** —— `assets/probe-shell.desktop` + `install-linux.sh`，并设置
...
[0.3.8]: https://github.com/OnlyChallgener/probe-shell/releases/tag/v0.3.8
[0.3.7]: https://github.com/OnlyChallgener/probe-shell/releases/tag/v0.3.7
[0.3.3]: https://github.com/OnlyChallgener/probe-shell/releases/tag/v0.3.3
[0.3.2]: https://github.com/OnlyChallgener/probe-shell/releases/tag/v0.3.2
[0.3.1]: https://github.com/OnlyChallgener/probe-shell/releases/tag/v0.3.1
[0.3.0]: https://github.com/OnlyChallgener/probe-shell/releases/tag/v0.3.0
[0.2.2]: https://github.com/OnlyChallgener/probe-shell/releases/tag/v0.2.2
